...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE manuales SET producto_id = ?, titulo = ?, url_manual = ?, tipo = ? WHERE id = ?";
    $stmt = mysqli_prepare($con, $sql);
    mysqli_stmt_bind_param($stmt, 'isssi', $producto_id, $titulo, $url_manual, $tipo, $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    // Si se subió una imagen, actualizar la ruta de la imagen
...
<div class="container mx-auto p-4">
    ...
    <form action="#" method="POST" enctype="multipart/form-data" class="mb-4">
        <div class="mb-4">
            ...
            <select id="producto_id" name="producto_id" required class="mt-1 block w-full border border-gray-300 rounded-md p-2     focus:border-blue-500 focus:ring-blue-500">
                ...
                <?php while ($row = mysqli_fetch_assoc($comboProductos)) : ?>
                    <option 
                        <?php echo ($producto_id == $row['id']) ? 'selected' : ''; ?>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="titulo" name="titulo" value="<?php echo htmlspecialchars($titulo); ?>" required class="mt-1 block w-full border border-gray-300 rounded-md p-2     focus:border-blue-500 focus:ring-blue-500">   
        </div>
        <div class="mb-4">
            <label for="url_manual" class="block text-sm font-medium text-gray-700">URL del Manual</label>
            <input type="text" id="url_manual" name="url_manual" value="<?php echo htmlspecialchars($url_manual); ?>" required class="mt-1 block w-full border border-gray-300 rounded-md p-2    focus:border-blue-500 focus:ring-blue-500">
        </div>
        <div class="mb-4">
        ...
        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Guardar Manual</button>
    </form>
</div>
